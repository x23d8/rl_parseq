"""Build the fresh Phase 12 holdout after its full power target is available.

The builder deliberately checks the unique-label/source pool before hashing or
rendering media and before creating its output directory. An underfilled pool
therefore leaves no partial holdout that could later be mistaken for a locked
...
from reinforcement_learning.phase_12_guarded_replicated_ppo.pool_status import (  # noqa: E402
    PHASE9,
    PHASE11,
    SHEET_ROW_DIRECTION,
    STABLE_SEED,
...
def validate_lock(path: Path) -> dict:
    ...
    if (
        ...
        or lock.get("status") != "prospective_locked_waiting_for_fresh_data"
        or lock.get("algorithm") != "guarded_replicated_candidate_oof_ppo_seed728"
    ):
        raise ValueError("Phase 12 candidate lock is invalid")
    ...
    if (
        ...
        raise ValueError("Phase 12 metadata guard differs from its locked rule")
    for field in ("policy_checkpoint", "action_registry"):
        ...
        if not artifact.is_file() or sha256_file(artifact) != entry.get("sha256"):
            raise ValueError(f"Locked Phase 12 {field} is unavailable or changed")
    return lock


def output_filename(index: int, source_id: str, suffix: str) -> str:
    clean = re.sub(r"[^0-9A-Za-z_-]+", "_", source_id).strip("_") or "sample"
    return f"phase12_{index:06d}_{clean}{suffix}"
...
def write_manifest(
    ...
    for index, (source, payload, suffix, digest, width, height) in enumerate(rendered, start=1):
        destination = images_dir / output_filename(index, str(source["source_id"]), suffix)
...
def run(args: argparse.Namespace) -> dict:
    ...
    opened = [Path(value).resolve() for value in args.opened_manifest]
    ...
    if output_dir != HERE and HERE not in output_dir.parents:
        raise ValueError("Phase 12 holdout artifacts must remain inside Phase 12")
    if output_dir.exists() and any(output_dir.iterdir()):
        raise FileExistsError(f"Refusing to overwrite locked Phase 12 holdout: {output_dir}")
    lock = validate_lock(lock_path)
    external = lock["external_contract"]
    power = lock["power_contract"]
    if (
        args.first_sheet_row != int(external["first_sheet_row_inclusive"])
        or external.get("sheet_row_direction") != SHEET_ROW_DIRECTION
        or args.minimum_samples != int(power["minimum_formal_samples"])
        or args.target_samples != int(power["target_samples"])
        or external.get("manual_acceptance_required") is not False
    ):
        raise ValueError("Phase 12 builder arguments differ from its prospective lock")
    ...
    # Fail before image hashing/rendering and before output_dir.mkdir().
    if len(candidates) < args.target_samples:
        raise ValueError(
            "Phase 12 fresh pool is below its power-locked target before media checks: "
            f"available={len(candidates)}, target={args.target_samples}, "
            f"shortage={args.target_samples - len(candidates)}"
        )
    ...
    if len(rendered) < args.target_samples:
        raise ValueError(
            "Phase 12 pool fell below its power target after media checks; no output was written: "
            f"valid={len(rendered)}, target={args.target_samples}"
        )

    output_dir.mkdir(parents=True, exist_ok=False)
    manifest, rows = write_manifest(output_dir, rendered)
    audit = {
        "contract": "phase12_fresh_locked_confirmatory_manifest_v1",
        "inference_run": False,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="append",
        default=[
            str(DEFAULT_HISTORICAL),
            str(DEFAULT_PHASE7_OPENED),
            str(DEFAULT_PHASE8_OPENED),
            str(PHASE9),
            str(PHASE11),
        ],
    )
    parser.add_argument(
        "--opened-source-manifest", action="append", default=[str(PHASE9), str(PHASE11)]
    )
